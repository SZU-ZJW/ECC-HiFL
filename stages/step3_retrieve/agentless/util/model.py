...
import logging
from agentless.util.api_requests import create_chatgpt_config, request_chatgpt_engine, query_sgl_answer, create_rm_query
...
qa_scoring_prompt = """
================================================================================
You are tasked with evaluating a problem and its corresponding predicted answer. 
...
================================================================================
PREDICTED MODIFICATION LOCATIONS:
{answer}

================================================================================
SCORING GUIDELINES:

1. Completeness Score: Measures recall — the proportion of actual required modification locations that are included in the prediction.
2. Accuracy Score: Measures precision — the proportion of predicted modification locations that are truly necessary.
3. Final Score: A combined score calculated as follows:

   Final Score = 0.8 * Completeness + (2 * Accuracy * Completeness) / (9 * Accuracy + Completeness)
...
qa_scoring_prompt_ir = """
================================================================================
You are tasked with evaluating a problem and its corresponding predicted irrelevant files/directories. 
Your evaluation will be based on the following criteria:
...
================================================================================
PREDICTED IRRELEVANT FILES/DIRECTORIES:
{answer}

================================================================================
SCORING GUIDELINES:

1. Accuracy Score: Measures safety — whether the predicted irrelevant files/directories contain any files that need to be modified.
   - Score = 1.0: If NONE of the predicted irrelevant files/directories contain files that need modification
   - Score ≈ 0.0: If ANY predicted irrelevant file/directory contains files that need modification
   - This is the MOST important metric

2. Completeness Score: Measures the reasonable prediction volume
   - Give higher scores for predictions that identify a reasonable number of irrelevant files
   - Give medium scores for very conservative predictions
   - Give lower scores for extremely limited predictions
   - Note: Sometimes, it's normal and acceptable if the prediction only covers a small percentage of the repository

3. Final Score: A combined score that heavily weights accuracy:

   Final Score = 0.9 * Accuracy + 0.1 * Completeness

All scores must be between 0 and 1, rounded to 2 decimal places.
...
class OpenAIChatDecoder(DecoderBase):
    ...
    def locgen(self, message: str, num_samples: int = 1, pred_list: list = None) -> List[dict]:
        ...
        inter_responses = pred_list[:num_samples]

        if inter_responses:
            completion_tokens = 0
            prompt_tokens = 0
            rm_query = create_rm_query(qa_scoring_prompt_ir, message, inter_responses)
            scores_list = query_sgl_answer(rm_query, self.logger)
            responses = [get_best_response(inter_responses, scores_list)]
        else:
            responses = [""]
            completion_tokens = 0
            prompt_tokens = 0
