...
"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.instance_pred_list = instance_pred_list
        self.max_tokens = 300
    ...
    def localize_irrelevant(self, top_n=1, mock=False):
        ...
        traj = model.locgen(message, num_samples=self.sample_num, pred_list=self.instance_pred_list)[0]
        traj["prompt"] = message
    ...
    def localize(self, top_n=1, mock=False) -> tuple[list, list, list, any]:
        ...
        traj = model.filegen(message, num_samples=self.sample_num, file_list=files, pred_list=self.instance_pred_list)[0]
        traj["prompt"] = message
        raw_output = traj["response"]
    ...
    def localize_function_from_compressed_files(
        ...
        message = ""
        traj = model.locgen(message, num_samples=self.sample_num, pred_list=self.instance_pred_list)[0]
        traj["prompt"] = self.instance_pred_list["prompt"]
        raw_output = traj["response"]
    ...
    def localize_line_from_coarse_function_locs(
        ...
        if generate_candidates:
            ...
            raw_trajs = model.linegen(message, num_samples=num_samples)
        else:
            model = make_model(
                model=self.model_name,
                backend=self.backend,
            ...
            message = ""
            raw_trajs = model.locgen_sk(
                message,
                num_samples=self.sample_num,
                pred_list=self.instance_pred_list,
            )
