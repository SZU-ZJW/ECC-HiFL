...
def load_pred_list(pred_list_file):
    pred_list_map = {}
    with open(pred_list_file, "r") as f:
        for line in f:
            data = json.loads(line)
            instance_id = data["instance_id"]
            pred_list = data["file_traj"]["all_responses"]
            prompt = data["file_traj"]["prompt"]
            metadata = {
                "all_responses": pred_list,
                "prompt": prompt,
            }
            pred_list_map[instance_id] = metadata
    return pred_list_map


def load_element_pred_list(pred_list_file):
    pred_list_map = {}
    with open(pred_list_file, "r") as f:
        for line in f:
            data = json.loads(line)
            instance_id = data["instance_id"]
            meta_data = {
                "response": data["related_loc_traj"][0]["all_responses"],
                "prompt": data["related_loc_traj"][0]["prompt"],
            }
                
            pred_list_map[instance_id] = meta_data
    return pred_list_map


def load_line_pred_list(pred_list_file):
    pred_list_map = {}
    with open(pred_list_file, "r") as f:
        for line in f:
            data = json.loads(line)
            instance_id = data["instance_id"]
            meta_data = {
                "response": data["edit_loc_traj"]["all_responses"][0],
                "prompt": data["edit_loc_traj"]["prompt"],
            }
            pred_list_map[instance_id] = meta_data
    return pred_list_map
...
def localize_irrelevant_instance(
    bug, args, swe_bench_data, existing_instance_ids, pred_list, sample_num, write_lock=None
):
    instance_id = bug["instance_id"]
    log_file = os.path.join(
        args.output_folder, "localization_logs", f"{instance_id}.log"
    )
    instance_pred_list = pred_list[instance_id]
...
def localize_instance(
    bug, args, swe_bench_data, start_file_locs, existing_instance_ids, pred_list, sample_num, write_lock=None
):
    instance_id = bug["instance_id"]
    log_file = os.path.join(
        args.output_folder, "localization_logs", f"{instance_id}.log"
    )
    if args.fine_grain_line_level and args.line_candidate_generation:
        instance_pred_list = pred_list.get(instance_id) if pred_list else None
    else:
        instance_pred_list = pred_list[instance_id]
    if args.target_id is not None:
        if args.target_id != bug["instance_id"]:
            ...
            for _ in range(MAX_RETRIES):
                ...
                if not args.direct_edit_loc:
                    ...
                    (
                        ...
                        keep_old_order=args.keep_old_order,
                        generate_candidates=args.line_candidate_generation,
                    )
                    additional_artifact_loc_edit_location = [
                        additional_artifact_loc_edit_location
                    ]
...
def localize_irrelevant(args):
    ...
    pred_list = load_pred_list(args.pred_list)
    ...
    if args.num_threads == 1:
        for bug in tqdm(swe_bench_data, colour="MAGENTA"):
            localize_irrelevant_instance(
                bug, args, swe_bench_data, existing_instance_ids, pred_list, sample_num
            )
    else:
        write_lock = Lock()
...
def localize(args):
    swe_bench_data = load_dataset(args.dataset, split="test")
    if args.related_level:
        pred_list = load_element_pred_list(args.pred_list)
    elif args.fine_grain_line_level and not args.line_candidate_generation:
        pred_list = load_line_pred_list(args.pred_list)
    elif args.fine_grain_line_level:
        pred_list = {}
    else:
        pred_list = load_pred_list(args.pred_list)
    ...
    if args.num_threads == 1:
        for bug in tqdm(swe_bench_data, colour="MAGENTA"):
            localize_instance(
                bug, args, swe_bench_data, start_file_locs, existing_instance_ids, pred_list, sample_num
            )
    else:
        write_lock = Lock()
        with concurrent.futures.ThreadPoolExecutor(
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Generate fresh fine-grained line candidates instead of selecting from --pred_list.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="q7bc",
        choices=["gpt-4o-2024-05-13", "deepseek-coder", "gpt-4o-mini-2024-07-18", "q7bc", "q32bc"],
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--pred_list",
        type=str,
        default="data/Exp/sample/results/swe-bench-lite/file_level/loc_outputs.jsonl",
        help="Current supported dataset for evaluation",
    )
    parser.add_argument(
        "--sample",
