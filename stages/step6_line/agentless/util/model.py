...
import logging
from agentless.util.api_requests import create_chatgpt_config, request_chatgpt_engine, query_sgl_answer, create_rm_query, query_q7bc, format_candidates_for_prompt, request_sglang_engine, create_sk_rm_query
...
class OpenAIChatDecoder(DecoderBase):
    ...
    def locgen_sk(self, message: str, num_samples: int = 1, pred_list: list = None) -> List[dict]:
        prompt = pred_list["prompt"]
        inter_responses  = pred_list["response"][:num_samples]
        

        if inter_responses:
            completion_tokens = 0
            prompt_tokens = 0
            rm_query = create_sk_rm_query(prompt, inter_responses)
            scores_list = request_sglang_engine(rm_query, self.logger)
    ...
    def linegen(self, message: str, num_samples: int = 1) -> List[dict]:
        ...
        if ret:
            ...
            if EP.sglang_rm:
                ...
                rm_query = create_sk_rm_query(message, inter_responses)
                scores_list = request_sglang_engine(rm_query, self.logger)
            else:
                scores_list = list(range(len(inter_responses), 0, -1))
