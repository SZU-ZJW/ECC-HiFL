...
def query_sgl_answer(questions, logger, num_answers=SAMPLE):
    sgl.set_default_backend(sgl.RuntimeEndpoint(EP.sglang_rm))
...
def query_q7bc(message, logger):
    client = OpenAI(
    base_url=EP.judge_base_url,
    api_key=EP.judge_api_key,
)
    response = client.chat.completions.create(
        model=EP.judge_model,
        messages=[
...
def _llama31_chat_template(messages):
    text = "<|begin_of_text|>"
    for msg in messages:
        text += (
...
def _apply_skywork_chat_template(prompts, logger):
    if EP.skywork_model_path:
        try:
            tokenizer = AutoTokenizer.from_pretrained(EP.skywork_model_path)
            return tokenizer.apply_chat_template(prompts, tokenize=False)
        except Exception as exc:
            logger.info(f"Skywork tokenizer unavailable, using built-in chat template: {exc}")
    return [
        prompt if isinstance(prompt, str) else _llama31_chat_template(prompt)
...
def request_sglang_engine(prompts, logger):
    prompts = _apply_skywork_chat_template(prompts, logger)

    url = _classify_url(EP.skywork_url)
    data = {"model": EP.skywork_model or "sk8b", "text": prompts}

    response = requests.post(url, json=data, timeout=120)
    response.raise_for_status()
    responses = response.json()
    if isinstance(responses, dict) and "data" in responses:
    ...
    for response in responses:
        if isinstance(response, dict) and "embedding" in response:
            ...
            results.append(float(embedding[0] if isinstance(embedding, list) else embedding))
        elif isinstance(response, dict) and "score" in response:
            results.append(float(response["score"]))
        else:
            results.append(float(response))

    logger.info(f"Final score: {results}")
    
    return results

def create_sk_rm_query(message: Union[str, list], response: list): 
    system_message = [
        {"role": "system", "content": "You are a helpful assistant."},
    ]
    info_requests = []
    for res in response:
        ...
        message_i = system_message + meta_message + answer
        info_requests.append(message_i)
