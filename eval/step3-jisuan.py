...
def process_experiment(modified_data, experiment_data, experiment_name):
    ...
    if count > 0:
        ...
        print(f"Exact Match Rate: ({exact_match_count}/{count})")
...
def main():
    ...
    for i in range(1, 11):
        file_path = f"./results/swe-bench-lite-{sample_num}-F-1@{i}/file_level/loc_outputs.jsonl"
        save_path = "./file_loc.jsonl"
        if os.path.exists(file_path):
            print(f"\nresults/{sample_num}/file_level/swe-bench-lite-{sample_num}-F-1@{i}...")
